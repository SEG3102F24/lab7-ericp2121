...
class ApiController(val authorRepository: AuthorRepository,
    ...
    ): ResponseEntity<CollectionModel<BookRepresentation>> {
        val pageable: Pageable = PageRequest.of(page, size)
        val books = bookRepository.findAll(pageable)

        return ResponseEntity(
            bookAssembler.toCollectionModel(books.content),
            HttpStatus.OK
        )
    }
    ...
    @Operation(summary = "Get author by ID by using detailed book information")
    @GetMapping("/authors/{id}/detailed-books")
    fun getAuthorWithDetailedBooks(@PathVariable("id") id: Long): ResponseEntity<AuthorRepresentation> {
        return authorRepository.findById(id)
            .map { author: Author ->
                val authorRep = authorAssembler.toModel(author)
                authorRep.books = author.books.map { bookAssembler.toModel(it) }
                ResponseEntity.ok(authorRep)
            }
            .orElse(ResponseEntity.notFound().build())
    }


    @Operation(summary = "Search for books by title or category")
    ...
    fun searchBooks(
        ...
        val books = when {
            !title.isNullOrEmpty() -> bookRepository.findByTitleContainingIgnoreCase(title)
            !category.isNullOrEmpty() -> bookRepository.findByCategoryContainingIgnoreCase(category)
            else -> listOf()
        }
        return ResponseEntity(
